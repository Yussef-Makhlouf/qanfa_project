...
const loginFormSchema = z.object({
  ...
  password: z.string().min(8, { message: "يجب أن تحتوي كلمة المرور على 8 أحرف على الأقل" }),
    ...
    >
      ...
      <DottedBorderContainer className="mb-6">
        ...
        <div className="mb-6 text-center">
          ...
          <h1 className="mt-2 text-center text-xl font-bold text-[#375A64]">مرحباً بعودتك!</h1>
          <p className="mt-1 text-center text-xs text-[#6A6A6A]">سوف تتلقى رسالة نصية للتحقق من حسابك.</p>
        </div>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4" dir="rtl">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="relative">
                  <div className="flex justify-between items-center">
                    <div className="w-8"></div>
                    <FormLabel className="text-xs font-medium text-[#375A64]">البريد الإلكتروني*</FormLabel>
                  </div>
                  <div className="relative">
                    <FormControl>
                      <BottomBorderInput
                        ...
                        icon={<Mail className="h-4 w-4 text-[#5A5A5A]" />}
                      />
                    </FormControl>
                  </div>
                  <FormMessage className="text-right text-[#FF3C3F] text-xs mt-1" />
            ...
            <div className="flex items-center justify-between pt-2">
              <Link
                ...
                className="text-xs text-[#375A64] hover:text-[#FF735D] hover:underline transition-colors"
              >
                هل نسيت كلمة المرور؟
              </Link>
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-center space-x-2 space-x-reverse">
                    <FormLabel className="text-xs text-[#5A5A5A] cursor-pointer">تذكرني</FormLabel>
                    <FormControl>
                      <Checkbox
                        checked={field.value}
                        onCheckedChange={field.onChange}
                        className="h-3.5 w-3.5 border-[#0000001A] data-[state=checked]:bg-[#FF735D] data-[state=checked]:border-[#FF735D]"
                      />
                    </FormControl>
                  </FormItem>
                )}
            ...
            >
              {isSubmitting ? "جاري التحقق..." : "استمرار"}
            </motion.button>
          </form>
        </Form>
      ...
      <div className="mt-4 flex justify-center gap-1 text-xs">
        ...
        <span className="text-[#5A5A5A]">ليس لديك حساب؟</span>
      </div>
